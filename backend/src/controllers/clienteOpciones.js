...
//funcion para registros en la bitacora
function insertlog (action, fk_id_log_category, fk_id_user){
    let sql=`INSERT INTO system_logs (action, date, fk_id_log_category, fk_id_user) VALUES ('${action}', (now()), '${fk_id_log_category}', '${fk_id_user}');`
    console.log(sql)
    conection.query(sql,(error, resultado, field)=>{
...
controller.editProduct=(req,res)=>{
    ...
    const{var_name, dou_price, text_description, 
        fk_id_product_category, fk_id_product_status, fk_id_department, fk_id_user}=req.body;
