const conection = require('../config/conexion')//requerimos la conexion a la BD 
...
function enviarCorreoOut(destinatario, codigo, res) {
    let config = nodemailer.createTransport({
        ...
        auth: {
            ...
            pass: process.env.secretPaswordO
        }
    });
    const opc = {
        from: process.env.secretEmailOutlook,
        subject: "Recuperacion de cuenta",
        to: `${destinatario}`,
        text: `Hola estimado usuario, a continuación te proporcionamos el código de verificación para el cambio de contraseña: "${codigo}".
Recuerda que este código solo se puede utilizar una vez.`
    };
...
controller.postUser = (req, res) => {
    const { fk_id_department, var_email, var_name, var_lastname, tex_password, bit_status, var_phone, id_user} = req.body
