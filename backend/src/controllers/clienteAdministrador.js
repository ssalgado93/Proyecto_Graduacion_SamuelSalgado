const conection = require('../config/conexion')
...
controller.updateUsuarios=(req,res)=>{
    const {bit_rol, id_user, var_name} =req.body;
    let sql=`UPDATE user SET bit_rol= "${bit_rol}" WHERE id_user=${id_user}`;
    let sql1=`SELECT * FROM user WHERE var_email="${var_name}"`
    ...
    conection.query(sql1,(err,rows)=>{
        if(err) res.json({status:"0", msg: err.sqlMessage})
            ...
            if(rows.length==0){
                ...
                res.json({status:"201", msg:"Ya existe el usuario"})
            }
            }
        })
}

controller.updateUsuarios2=(req,res)=>{
    const {bit_status, id_user, var_name} =req.body;
    let sql=`UPDATE user SET bit_status= "${bit_status}" WHERE id_user=${id_user}`;
    let sql1=`SELECT * FROM user WHERE var_email="${var_name}"`
    console.log(sql1, sql);
    conection.query(sql1,(err,rows)=>{
        if(err) res.json({status:"0", msg: err.sqlMessage})
...
controller.getOneUser2=(req,res)=>{
    const{id_user}=req.params
    let sql=`SELECT * from user WHERE id_user=${id_user}`
