 ...
 //public date:string=new Date().toLocaleString()
  ...
  addWihlist(id:string,item:traerProducto){
    Swal.fire({
      position: 'center',
      icon: 'success',
      title: 'La publicación se ha agregado a la lista de deseos',
      showConfirmButton: false,
      timer: 1500
    })
  ...
  deleteWihlist(id:string,item:traerProducto){
    Swal.fire({
      position: 'center',
      icon: 'success',
      title: 'La publicación se ha removido de la lista de deseos',
      showConfirmButton: false,
      timer: 1500
    })
    let data:deleteWishlist = {
...
newChat(idUser:number,idProduct:number){
  ...
  this.WebSocketsService.listen("newchatresponse").subscribe((data:any)=>{
    if (data.status=='200'||data.status=='202') this.router.navigate([`navegacionProductos/chats/${data.id_chat}`])
  })
}
}
