...
export class NavigationProductsComponent implements OnInit {
  ...
  toDisplay=true
  ruta:string = "inicio/terminosycondiciones"//ruta para terminos y condicions
  ngOnInit(): void {
    ...
    if (localStorage.getItem('token')!=null && localStorage.getItem('token') != ""){//vericamos que existe un token en localstorage
      ...
      this.router.navigate([`/navegacionProductos`])//en caso de que no exista un token se de volvera al login
    }
  }
